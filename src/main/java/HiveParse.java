...
public class HiveParse {
    ...
    private final Map<String, String> cols = new TreeMap<String, String>();
    private final Map<String, String> colAlais = new TreeMap<String, String>();
    private final Set<String> tables = new HashSet<String>();
    private final Stack<String> tableNameStack = new Stack<String>();
    private final Stack<Oper> operStack = new Stack<Oper>();
...
//                    nowQueryTable = nowQueryTable
    ...
    private void output(Map<String, String> map) {
        ...
        while (it.hasNext()) {
            String key = it.next();
            if((key.toUpperCase().contains(Dbna.SD_SRC.name())
                    ||key.toUpperCase().contains(Dbna.SD_ODS.name())
                    ||key.toUpperCase().contains(Dbna.SD_EDW.name()))
                    && key.toUpperCase().contains("TS_KD")
            ) {
                System.out.println(key.toUpperCase() + "\t" + map.get(key).toUpperCase());
            }
        }
    }

    public void parseTable(ASTNode ast) {
        parseIteral(ast);

        for (String table : tables) {
            //System.out.println("***************表***************");
            if(table.toUpperCase().contains(Dbna.SD_SRC.name())||table.toUpperCase().contains(Dbna.SD_ODS.name())||table.toUpperCase().contains(Dbna.SD_EDW.name())){
    ...
    public void parseCol(ASTNode ast) {
        parseIteral(ast);
        System.out.println("***************列***************");
        output(cols);
    }
    public void parseAlias(ASTNode ast) {
        parseIteral(ast);
